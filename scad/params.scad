...
// =====================================================
// 축 계통 변형 (axle variant) — 렌더 시 -D 'axle="collar"' 등으로 전환
//  "shoulder" : 현행 — 스트리퍼 볼트 Ø8xL55xM6 + 심 + M6 나일록 (정밀도 기준)
//  "collar"   : 옵션2 — Ø8 연마봉 + 샤프트 칼라 2개 (심·너트·와셔 삭제, 무단 예압)
//  "printed"  : 옵션5 — M8x70 부분나사 볼트 + 프린팅 슬리브 (금속 스페이서 삭제)
// 세 변형 모두 베어링 z(20..27 / 37..44)와 허브·풀리·클램프는 동일
// =====================================================
axle = "shoulder";
...
// ---- [shoulder] 스트리퍼 볼트 Ø8 쇼울더 L55 x M6 (나사부 10mm) ----
...
axle_hole_d   = 8.3;                    // 베이스 축 구멍 (Ø8/M8 공통)

// ---- [collar] Ø8 연마봉 + 샤프트 칼라 (ID8 OD16 W8, 세트스크류식) ----
rod_l    = 70;  rod_z0 = -17;           // 봉 z -17..53
collar_od = 16; collar_w = 8;           // 하부 칼라 -16..-8, 상부 칼라 44..52

// ---- [printed] M8x70 부분나사 볼트 (DIN931: 나사 22, 민자 48) ----
m8_head_d = 13;   m8_head_h = 5.3;
m8_shank_l = 48;                        // 민자부 -9.6..38.4
nut_m8_af = 13;   nut_m8_h = 8;         // M8 나일록 (DIN985)
sleeve_standoff_od = 12.5;              // 프린팅 스탠드오프 슬리브 (z 0..20)
sleeve_inner_od    = 12;                // 프린팅 내륜 슬리브 (z 27..37)
...
// bore: 내부 통과물 클리어런스 — 금속 스페이서(OD10)는 12, 프린팅 슬리브(OD12.5)는 14
p60_bore = (axle == "printed") ? 14 : 12;
...
// ---- 색상 — 회전부(주황 계열) vs 고정부(블루/그레이 계열) ----
c_stat   = [0.36, 0.46, 0.62];          // 고정 프린팅 부품 (베이스)
c_steel  = [0.75, 0.76, 0.78];          // 고정 금속 (축·내륜·체결류)
c_alu    = [0.85, 0.86, 0.88];          // 고정 금속 (스페이서·와셔)
c_move   = [0.93, 0.53, 0.12];          // 회전 프린팅 (암 허브/암)
c_move2  = [1.00, 0.72, 0.35];          // 회전 프린팅 (클램프 링)
c_move_m = [0.88, 0.63, 0.30];          // 회전 금속 (풀리·외륜·클램프 볼트)
