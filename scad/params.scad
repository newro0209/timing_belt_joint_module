...
// 축 계통은 스틸 계열 톤온톤: 너트/칼라(진함) < 축(중간) < 내륜 < 스페이서 < 와셔(밝음)
c_stat   = [0.36, 0.46, 0.62];          // 고정 프린팅 부품 (베이스·슬리브)
c_steel  = [0.75, 0.76, 0.78];          // 고정 금속 일반 (내륜·모터 볼트 등)
c_axle   = [0.60, 0.62, 0.67];          // 축 (스트리퍼 볼트/연마봉/M8 볼트)
c_nut    = [0.48, 0.51, 0.57];          // 너트·샤프트 칼라 (가장 진함)
c_washer = [0.93, 0.94, 0.96];          // 평와셔 (가장 밝음)
c_shim   = [0.70, 0.74, 0.84];          // 심 와셔 (청회색)
c_alu    = [0.85, 0.86, 0.88];          // 스페이서
