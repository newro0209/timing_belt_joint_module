...
module base_plate() {
    ...
    difference() {
        union() {
            ...
            // 베어링 보스 (하향, 잭 블록과 같은 방식으로 서포트 프리 출력)
        ...
        translate([0, 0, brgA_z0 - 0.01])
            cylinder(d = brg_od + 0.1, h = brg_w + 1);
        // 외륜 웹 내경 Ø20 (z -17..-7) — 내륜 스페이서 회전 간극,
        // 웹 상·하면이 외륜을 양방향 축하중에 대해 받친다
        translate([0, 0, spacer_z0 - 0.01])
            cylinder(d = 20, h = web_h + 0.02);
        // 하부 608 포켓 Ø22.1 (보스 하면에서 압입, z -24..-17 + 입구 여유)
        translate([0, 0, boss_z0 - 1])
            cylinder(d = brg_od + 0.1, h = (brgB_z0 + brg_w) - boss_z0 + 1.01);
...
//    이빨부 z 8.9..16.5 — 벨트(9.7..15.7)와 정렬
//    상단이 축 끝(14) 위로 약간 돌출하나 풀리 강성으로 문제없음
// -----------------------------------------------------
module pulley20() {
    tinted(c_move_m)
